# proyecto_2marcos
proyecto 2
# 1. Instalación forzada de SBCL
!apt-get update -qq && apt-get install -y sbcl -qq

# 2. Escritura limpia del archivo
lisp_code = '''
...
(defun calcular-clase (e s c)
  (let* ((filas (remove-if-not (lambda (x) (= (third x) c)) *dataset*))
         ...
         (m-s (promedio salario s)) ; Error tipográfico corregido aquí
